...
def year_folder(year, path, log_handler):
    if not year.is_dir():
        log_handler(f"Unexpected file {year.name} in {str(path)} \n")
        return False
    elif not year.name.isdigit():
        log_handler(f"Unexpected folder {year.name} in {str(path)} \n")
        return False
    else:
        return True
...
def issue_folder(issue, path, log_handler):
    ...
    if not issue.is_dir():
        log_handler(f"Unexpected file {issue.name} in {str(path)} \n")
        return False
    elif not issue_name_pattern.match(issue_folder):
        log_handler(f"Unexpected folder {issue.name} in {str(path)} \n")
        return False
    else:
        return True
...
def file_type_folders(folder, path, log_handler):
    ...
    if not folder.is_dir():
        log_handler(f"Unexpected file {folder.name} in {str(path)} \n")
        return False
    elif folder.name not in folders:
        log_handler(f"Unexpected folder {folder.name} in {str(path)} \n")
        return False
    else:
        return True
...
def file_types(file, path, extensions, log_handler):
    if file.suffix not in extensions:
        log_handler(f"Unexpected file {file.name} in {str(path)} \n")
        return False
    else:
        return True
